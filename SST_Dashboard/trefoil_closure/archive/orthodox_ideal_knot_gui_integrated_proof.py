...
def scan_bs_energy(geom: GeometryData, use_torch: bool, cancel_cb: Optional[Callable[[], bool]] = None, progress_cb: Optional[Callable[[float, str], None]] = None) -> Tuple[np.ndarray, np.ndarray]:
...
class BenchmarkWorker(QThread):
    ...
    def run(self):
        try:
            ...
            self.log_signal.emit(log_prefix(f"[META] family_tags={','.join(family_tags(self.entry)) or '-'}"))
            self.log_signal.emit(log_prefix(f"[META] r_ref = Gamma_ref/(2*pi*U_ref) = {r_ref:.8e} m"))
            geom = build_geometry(self.entry, self.points_per_component, self.int_points_per_component, cancel_cb=self.isInterruptionRequested, progress_cb=lambda f, phase: self._emit_progress(0.05 + 0.35 * f, phase))
            ...
            a_scan, E_vals = scan_bs_energy(geom, self.use_torch, cancel_cb=self.isInterruptionRequested, progress_cb=lambda f, phase: self._emit_progress(0.40 + 0.54 * f, phase))
...
class MainWindow(QMainWindow):
    ...
    def _setup_ui(self):
        ...
        self.tabs.addTab(self.theorem_box, "Proof / Continuation")
    ...
    def update_theorem_tab(self, result: Optional[dict] = None):
        entry = self.current_entry()
        html = []
        html.append("<h3>Proof / Continuation</h3>")
        html.append(r"""<pre>A_{\mathrm{req}} = 1/(4\pi)
        a_{\mathrm{nc}} = \sqrt{A_K/\pi}\;\Gamma_{\mathrm{ref}}/U_{\mathrm{ref}}
        r_{\mathrm{ref}} = \Gamma_{\mathrm{ref}}/(2\pi U_{\mathrm{ref}})
        a_{\mathrm{nc}}/r_{\mathrm{ref}} = \sqrt{4\pi A_K}</pre>""")
        ...
        if result and not result.get('cancelled'):
            ...
            chosen = None
            for fr in fits:
                if fr.get('method') == 'plateau_0.12':
                    chosen = fr
                    break
            if chosen is None and fits:
                chosen = min(fits, key=lambda fr: abs(fr.get('A_K', float('nan')) - A_REQ) if math.isfinite(fr.get('A_K', float('nan'))) else 1e9)
            if chosen is not None:
                A_K = float(chosen['A_K'])
                ratio = A_K / A_REQ
                anc_over = math.sqrt(max(0.0, 4.0 * math.pi * A_K))
                resid = anc_over - 1.0
                html.append("<p><b>Chosen fit for theorem summary:</b> " + chosen['method'] + "</p>")
                ...
                html.append(f"<tr><td>a_nc / r_ref</td><td>{anc_over:.6f}</td></tr>")
                html.append(f"<tr><td>closure residual (a_nc/r_ref - 1)</td><td>{resid:+.6e}</td></tr>")
                html.append("</table>")
                html.append("<p><b>Interpretation.</b> In the orthodox framing, the clean theorem candidate is not ")
                html.append("a full physical core-radius claim, but the numerical proximity of the ideal-knot logarithmic coefficient to ")
                html.append("<code>1/(4π)</code>, together with the induced no-contact scale ratio <code>a_nc/r_ref</code>.</p>")
            ...
            html.append("<p><i>Run the benchmark to populate theorem / continuation diagnostics.</i></p>")
