#!/usr/bin/env python3
"""
SST ideal-trefoil robustness sweep v6
=====================================
...
OUTPUT_DIR = os.path.abspath(os.path.join(SCRIPT_DIR, "sst_ideal_trefoil_robustness_outputs_v6"))
KNOT_ID = "3:1:1"
...
def write_best_estimate(rows: List[dict], outdir: str) -> None:
    ...
    csv_path = os.path.join(outdir, "final_best_estimate_v6.csv")
    with open(csv_path, "w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(f, fieldnames=list(result.keys()))
        writer.writeheader()
        writer.writerow(result)

    txt_path = os.path.join(outdir, "final_best_estimate_v6.txt")
    with open(txt_path, "w", encoding="utf-8") as f:
        f.write("SST ideal-trefoil final best-estimate v6\n")
        f.write("=" * 64 + "\n")
    ...
    tex_path = os.path.join(outdir, "final_best_estimate_v6.tex")
    with open(tex_path, "w", encoding="utf-8") as f:
        f.write("\\paragraph{Final best-estimate v6.}\n")
        if best:
        ...
        f.write("Interpretation: numerical support for percent-level closure; v6 extends v5 with theorem/continuation/barrier diagnostics and a shift-free postcheck.\n")
...
def save_shiftfree_postcheck_plot(shift_rows: List[dict], outdir: str) -> None:
    ...
    plt.legend(fontsize=7, ncol=2)
    plt.tight_layout()
    plt.savefig(os.path.join(outdir, "summary_astar_over_rc_vs_lambda_shiftfree_v6.png"), dpi=170, bbox_inches="tight")
    plt.close(fig)


def run_v6_shiftfree_postcheck(rows: List[dict], outdir: str) -> List[dict]:
    """
    Reuse the already-extracted sweep fit data, but rerun the root problem with a
    ...
    print("[V6] Running post-sweep shift-free barrier postcheck")
    print("[V6] contact_model={} fit_method={} seed_rows={}".format(
        SHIFTFREE_POSTCHECK_CONTACT_MODEL, PREFERRED_FIT_METHOD, len(seeds)
    ))
    ...
    fb_mode = "lowest_energy" if BRANCH_MODE == "lowest_energy" else "closest_to_a_nc"
    ...
    for seed in seeds:
        ...
        for lam_K in lam_iter_template:
            ...
            candidates = find_root_candidates(
                A_K, a_K, L_dimless, d_min_dimless, lam_K, p_exp, SHIFTFREE_POSTCHECK_CONTACT_MODEL
            )
            a_star, is_min, E_star, root_count, min_root_count, root_choice = choose_root_continuation(
                candidates, prev_a_star, a_nc, fallback_mode=fb_mode
            )
            ...
            if np.isfinite(a_star):
                prev_a_star = float(a_star)

            beta_dimless = float(seed["d_min_dimless"]) * L_natural / r_c_canon
            contact_force_at_x1 = contact_force_dimensionless(
                1.0, beta_dimless, lam_K, p_exp, SHIFTFREE_POSTCHECK_CONTACT_MODEL
            )
            F_at_x1 = F_dimensionless(
                1.0, A_K, beta_dimless, lam_K, p_exp, SHIFTFREE_POSTCHECK_CONTACT_MODEL
            )
            ...
            continuation_jump = (
                ...
                if np.isfinite(a_star_over_rc) and np.isfinite(prev_val)
                else np.nan
            )
            ...
            row = {
                ...
                "contact_model": SHIFTFREE_POSTCHECK_CONTACT_MODEL,
            ...
            print("[V6] N_geom={} N_int={} fit={} lambda_K={} F_at_x1={:.8e} contact_force_at_x1={:.8e} a_star_over_rc={} delta_vs_legacy={}".format(
                row["N_geom"], row["N_int_actual"], row["fit_method"], row["lambda_K"],
                row["F_at_x1"], row["contact_force_at_x1"], row["a_star_over_rc"], row["delta_a_star_vs_legacy"]
            ))

    csv_path = os.path.join(outdir, "shiftfree_postcheck_v6.csv")
    ...
    txt_path = os.path.join(outdir, "shiftfree_postcheck_v6.txt")
    with open(txt_path, "w", encoding="utf-8") as f:
        f.write("V6 post-sweep shift-free barrier theorem check\n")
        f.write("=" * 64 + "\n")
        f.write(f"Derived from legacy sweep rows only; no legacy outputs were modified.\n")
        f.write(f"Preferred fit method       : {PREFERRED_FIT_METHOD}\n")
        f.write(f"Shift-free contact model   : {SHIFTFREE_POSTCHECK_CONTACT_MODEL}\n")
    ...
    tex_path = os.path.join(outdir, "shiftfree_postcheck_v6.tex")
    with open(tex_path, "w", encoding="utf-8") as f:
        f.write("\\paragraph{V6 post-sweep shift-free barrier check.}\n")
...
def write_run_metadata(outdir: str, mode: str, backend_mode: str, backend_used: str,
    ...
    path = os.path.join(outdir, "run_metadata_v6.json")
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="SST Robustness Sweep V6")
    ...
    parser.add_argument("--output-dir", default="", help="Output directory (default: .../sst_ideal_trefoil_robustness_outputs_v6)")
    ...
    print("[META] SST ROBUSTNESS SWEEP V6 - mode={} knot_id={}".format(args.mode.upper(), KNOT_ID))
    ...
    csv_path = os.path.join(OUTPUT_DIR, "robustness_summary_v6_{}.csv".format(args.mode))
    ...
    print("[META] Best-estimate v6 CSV/TXT/TeX : {}".format(OUTPUT_DIR))
    if RUN_V6_SHIFT_FREE_POSTCHECK:
        print("[META] Shift-free postcheck v6 CSV/TXT/TeX : {}".format(OUTPUT_DIR))
    print("[META] run_metadata_v6.json : {}".format(os.path.join(OUTPUT_DIR, "run_metadata_v6.json")))
