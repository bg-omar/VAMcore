#!/usr/bin/env python3
"""
SST ideal-trefoil robustness sweep v4 (maintainable + backend + timing + metadata)
===============================================================================

Based on run3 with:
- Single backend selector (auto / local_cpp_scan / torch / numpy).
- Consolidated project integration (ideal.txt, local sst_core).
- Timing instrumentation and run_metadata_v4.json.
- Best-estimate practical + asymptotic; v4 output filenames.
- Parse-friendly log prefixes [META], [BACKEND], [GEOM], [SCAN], [FIT], [ROOT], [BEST], [TIME].
- CLI presets (fast/full) and optional args.
- Physics unchanged; a_nc naming preserved.
"""
...
OUTPUT_DIR = os.path.abspath(os.path.join(SCRIPT_DIR, "sst_ideal_trefoil_robustness_outputs_v4"))
KNOT_ID = "3:1:1"
...
def write_best_estimate(rows: List[dict], outdir: str) -> None:
    ...
    result["comment"] = "Practical = preferred no-contact run. Asymptotic = midpoint and half-range from two highest N_int at preferred fit, lambda=0."

    csv_path = os.path.join(outdir, "final_best_estimate_v4.csv")
    with open(csv_path, "w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(f, fieldnames=list(result.keys()))
        writer.writeheader()
        writer.writerow(result)

    txt_path = os.path.join(outdir, "final_best_estimate_v4.txt")
    with open(txt_path, "w", encoding="utf-8") as f:
        f.write("SST ideal-trefoil final best-estimate v4\n")
        f.write("=" * 64 + "\n")
    ...
    tex_path = os.path.join(outdir, "final_best_estimate_v4.tex")
    with open(tex_path, "w", encoding="utf-8") as f:
        f.write("\\paragraph{Final best-estimate v4.}\n")
        if best:
        ...
        f.write("Interpretation: numerical support for percent-level closure.\n")
...
def write_run_metadata(outdir: str, mode: str, backend_mode: str, backend_used: str,
    ...
    path = os.path.join(outdir, "run_metadata_v4.json")
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="SST Robustness Sweep V4")
    ...
    parser.add_argument("--output-dir", default="", help="Output directory (default: .../sst_ideal_trefoil_robustness_outputs_v4)")
    ...
    print("[META] SST ROBUSTNESS SWEEP V4 - mode={} knot_id={}".format(args.mode.upper(), KNOT_ID))
    ...
    csv_path = os.path.join(OUTPUT_DIR, "robustness_summary_v4_{}.csv".format(args.mode))
    ...
    print("[META] Best-estimate v4 CSV/TXT/TeX : {}".format(OUTPUT_DIR))
    print("[META] run_metadata_v4.json : {}".format(os.path.join(OUTPUT_DIR, "run_metadata_v4.json")))
