...
V10.2 addition:
- optional automatic UTF-8 console mirror log (stdout+stderr) under the output directory; see --console-log / --no-console-log.
"""
...
PARALLEL_SCOPE = "auto"   # auto | none | geom
...
OUTPUT_DIR = os.path.abspath(os.path.join(SCRIPT_DIR, "sst_knot_candidate_robustness_outputs_v10_2"))
...
def write_knot_comparison_summary(records: List[dict], outdir: str) -> None:
    ...
    df = df[cols]
    csv_path = os.path.join(outdir, "knot_candidate_comparison_v10_1.csv")
    txt_path = os.path.join(outdir, "knot_candidate_comparison_v10_1.txt")
...
def write_run_metadata(outdir: str, mode: str, backend_mode: str, backend_used: str,
    ...
    path = os.path.join(outdir, "run_metadata_v10_2.json")
...
def prompt_cli_menu_v10() -> List[str]:
    ...
    knot_preset = _prompt_text("Knot preset/group (blank = none; e.g. leptons, quarks, twist_ladder)", "")
    ...
    parallel_scope = _prompt_choice("Parallel scope", ["auto", "none", "geom"], "auto")
...
def _compute_geom_config_result(payload: Dict[str, object]) -> Dict[str, object]:
    ...
    return {"rows": rows, "plateau_diag_rows": plateau_diag_rows, "summary": summary}
...
def run_single_knot_sweep(args: argparse.Namespace, mode: str, knot_id: str, output_dir: str, run_start_wall: float, base_output_dir: str) -> Dict[str, object]:
    ...
    if EMIT_V5_LOGS:
        ...
        print("[THEOREM] v10.2 parallel_scope={} max_workers={}".format(PARALLEL_SCOPE, MAX_WORKERS if MAX_WORKERS else "auto"))
    ...
    csv_path = os.path.join(OUTPUT_DIR, "robustness_summary_v10_2_{}.csv".format(mode))
    write_csv(rows, csv_path)
    if SAVE_SUMMARY_PLOTS:
        save_summary_plots(rows, OUTPUT_DIR)
    if SAVE_PLATEAU_DIAGNOSTICS:
        plateau_csv = os.path.join(OUTPUT_DIR, "plateau_mask_diagnostics_v10_2.csv")
        write_plateau_diagnostics_csv(plateau_diag_rows, plateau_csv)
        save_plateau_diagnostics_summary_plot(plateau_diag_rows, os.path.join(OUTPUT_DIR, "plateau_mask_summary_v10_2.png"))
    ...
    print("[META] run_metadata_v10_2.json : {}".format(os.path.join(OUTPUT_DIR, "run_metadata_v10_2.json")))
...
def main() -> None:
    ...
    parser.add_argument("--output-dir", default="", help="Output directory (default: .../sst_knot_candidate_robustness_outputs_v10_2)")
    ...
    parser.add_argument("--parallel-scope", choices=["auto", "none", "geom"], default="auto", help="Parallel execution: auto | none | geom")
    parser.add_argument("--max-workers", type=int, default=0, help="Process pool size for geom-parallel mode; 0 = auto")
        ...
        try:
            if args.console_log:
                ...
                log_path = os.path.join(
                    base_output_dir,
                    "console_log_{}.log".format(time.strftime("%Y%m%d_%H%M%S")),
                )
            ...
            if _TORCH_AVAILABLE and torch is not None:
                try:
                    torch_line = "torch={} cuda_available={}".format(
    ...
    try:
        ...
        print("[META] v10.2 knot batch = {}".format(knot_ids))
