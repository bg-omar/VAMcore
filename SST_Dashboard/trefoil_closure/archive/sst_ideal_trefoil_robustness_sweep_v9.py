...
OUTPUT_DIR = os.path.abspath(os.path.join(SCRIPT_DIR, "sst_ideal_trefoil_robustness_outputs_v9"))
KNOT_ID = "3:1:1"
...
def write_best_estimate(rows: List[dict], outdir: str) -> None:
    ...
    csv_path = os.path.join(outdir, "final_best_estimate_v9.csv")
    with open(csv_path, "w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(f, fieldnames=list(result.keys()))
        writer.writeheader()
        writer.writerow(result)

    txt_path = os.path.join(outdir, "final_best_estimate_v9.txt")
    ...
    tex_path = os.path.join(outdir, "final_best_estimate_v9.tex")
...
def write_v8_extrapolation(rows: List[dict], postcheck_rows: List[dict], outdir: str) -> None:
    ...
    # 3) Write CSV
    # ------------------------------------------------------------
    csv_path = os.path.join(outdir, "extrapolation_summary_v9.csv")
    if out_rows:
    ...
    # ------------------------------------------------------------
    txt_path = os.path.join(outdir, "final_theorem_report_v9.txt")
    tex_path = os.path.join(outdir, "final_theorem_report_v9.tex")
    ...
    with open(txt_path, "w", encoding="utf-8") as f:
        f.write("SST ideal-trefoil theorem-style extrapolation report v9\n")
        f.write("=" * 72 + "\n")
    ...
    with open(tex_path, "w", encoding="utf-8") as f:
        f.write("\paragraph{v9 asymptotic extrapolation.}\n")
        if A_fit is not None and anc_fit is not None and A_tail is not None and anc_tail is not None:
            f.write(
            ...
            f.write(
                f"obtaining $A_\infty={A_fit['y_inf']:.10f}$ with "
                f"$A_\infty-1/(4\pi)={A_fit['y_inf_minus_target']:.3e}$, and "
                f"$(a_{{\mathrm{{nc}}}}/r_c)_\infty={anc_fit['y_inf']:.10f}$ with "
                f"$(a_{{\mathrm{{nc}}}}/r_c)_\infty-1={anc_fit['y_inf_minus_target']:.3e}$. "
            )
            f.write(
                f"The observed high-resolution tail is quasi-monotone toward the target for $A_K$ "
                f"({ 'yes' if A_tail['tail_quasi_monotone'] else 'no' }) and for $a_{{\mathrm{{nc}}}}/r_c$ "
                f"({ 'yes' if anc_tail['tail_quasi_monotone'] else 'no' }). "
            )
        if lam0_astar_fit is not None and lam0_F_fit is not None and lam0_astar_tail is not None and lam0_F_tail is not None:
            f.write(
                f"For the shift-free postcheck at $\lambda_K=0$, "
                f"$(a^*/r_c)_\infty={lam0_astar_fit['y_inf']:.10f}$ with deviation "
                f"{lam0_astar_fit['y_inf_minus_target']:.3e}, and $F(1)_\infty={lam0_F_fit['y_inf']:.3e}$. "
            )
            f.write(
...
def run_v6_shiftfree_postcheck(rows: List[dict], outdir: str) -> List[dict]:
    ...
    try:
        ...
        csv_path = os.path.join(outdir, "shiftfree_postcheck_v9.csv")
        ...
        txt_path = os.path.join(outdir, "shiftfree_postcheck_v9.txt")
        with open(txt_path, "w", encoding="utf-8") as f:
        ...
        tex_path = os.path.join(outdir, "shiftfree_postcheck_v9.tex")
        with open(tex_path, "w", encoding="utf-8") as f:
...
def write_run_metadata(outdir: str, mode: str, backend_mode: str, backend_used: str,
    ...
    path = os.path.join(outdir, "run_metadata_v9.json")
...
def prompt_cli_menu_v9() -> List[str]:
    print("\nSST Robustness Sweep V9 interactive launcher")
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="SST Robustness Sweep V9")
    ...
    parser.add_argument("--output-dir", default="", help="Output directory (default: .../sst_ideal_trefoil_robustness_outputs_v9)")
    ...
    print("[META] SST ROBUSTNESS SWEEP V9 - mode={} knot_id={}".format(args.mode.upper(), KNOT_ID))
    ...
    print("[META] Best-estimate v9 CSV/TXT/TeX : {}".format(OUTPUT_DIR))
    if RUN_V6_SHIFT_FREE_POSTCHECK:
        print("[META] Shift-free postcheck v9 CSV/TXT/TeX : {}".format(OUTPUT_DIR))
    print("[META] run_metadata_v9.json : {}".format(os.path.join(OUTPUT_DIR, "run_metadata_v9.json")))
