#!/usr/bin/env python3
"""
SST ideal-trefoil robustness sweep v8
=====================================
...
OUTPUT_DIR = os.path.abspath(os.path.join(SCRIPT_DIR, "sst_ideal_trefoil_robustness_outputs_v8"))
KNOT_ID = "3:1:1"
...
EXTRAP_PREFERRED_FIT_METHOD = "plateau_0.12"   # normally same as PREFERRED_FIT_METHOD
...
def write_best_estimate(rows: List[dict], outdir: str) -> None:
    ...
    csv_path = os.path.join(outdir, "final_best_estimate_v8.csv")
    with open(csv_path, "w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(f, fieldnames=list(result.keys()))
        writer.writeheader()
        writer.writerow(result)

    txt_path = os.path.join(outdir, "final_best_estimate_v8.txt")
    with open(txt_path, "w", encoding="utf-8") as f:
        f.write("SST ideal-trefoil final best-estimate v8\n")
        f.write("=" * 64 + "\n")
    ...
    tex_path = os.path.join(outdir, "final_best_estimate_v8.tex")
    with open(tex_path, "w", encoding="utf-8") as f:
        f.write("\\paragraph{Final best-estimate v8.}\n")
        if best:
        ...
        f.write("Interpretation: numerical support for percent-level closure; v8 inherits v7 dimensionless-x root finding plus v6 theorem/continuation/barrier diagnostics, shift-free postcheck, and optional v8 extrapolation reports.\n")
...
def _score_row_for_nint_choice(r: dict, prefer_root_choice: Optional[str] = None) -> Tuple[float, float, float]:
    """
    Larger is better.
    1) prefer larger N_geom
    2) optionally prefer a specific root_choice
    3) then prefer smaller |F_at_x1|
    """
    Ng = float(r.get("N_geom", 0))
    rc_bonus = 1.0 if (prefer_root_choice is not None and r.get("root_choice") == prefer_root_choice) else 0.0
...
def pick_best_row_per_nint(rows_local: List[dict], prefer_root_choice: Optional[str] = None) -> List[dict]:
    """
...
def write_v8_extrapolation(rows: List[dict], postcheck_rows: List[dict], outdir: str) -> None:
    """
    Writes:
      - extrapolation_summary_v8.csv
      - final_theorem_report_v8.txt
      - final_theorem_report_v8.tex

    Uses:
      * main sweep rows for A_K and a_nc/r_c at lambda=0
      * shift-free postcheck rows for a*/r_c(lambda) and F(1)(lambda)
    """
    ...
    if len(main_nc) >= 2:
        ...
        out_rows.append(_make_extrap_row(
            "main_no_contact", "A_K", fit_method, 0.0, n_vals, A_vals, A_fit, A_req
        ))

        anc_vals = [float(r["a_nc_over_rc"]) for r in main_nc]
        anc_fit = fit_powerlaw_extrapolation(n_vals, anc_vals, target=1.0)
        out_rows.append(_make_extrap_row(
            "main_no_contact", "a_nc_over_rc", fit_method, 0.0, n_vals, anc_vals, anc_fit, 1.0
        ))

    # ------------------------------------------------------------
    # 2) Shift-free postcheck extrapolation from v7 targeted_x_nc rows
    # ------------------------------------------------------------
    post_sf = [
        r for r in postcheck_rows
        if r.get("fit_method") == fit_method
        and r.get("contact_model") == "barrier_flat_at_rc"
        and r.get("root_choice") == "targeted_x_nc"
    ...
    if post_sf:
        ...
        for lam in lambda_vals:
            ...
            subset = pick_best_row_per_nint(subset, prefer_root_choice="targeted_x_nc")
            if len(subset) < 2:
                continue
            ...
            out_rows.append(_make_extrap_row(
                "shiftfree_postcheck", "a_star_over_rc", fit_method, lam, n_vals, astar_vals, astar_fit, 1.0
            ))

            Fx_vals = [float(r["F_at_x1"]) for r in subset]
            Fx_fit = fit_powerlaw_extrapolation(n_vals, Fx_vals, target=0.0)
            out_rows.append(_make_extrap_row(
                "shiftfree_postcheck", "F_at_x1", fit_method, lam, n_vals, Fx_vals, Fx_fit, 0.0
            ))
    ...
    # 3) Write CSV
    # ------------------------------------------------------------
    csv_path = os.path.join(outdir, "extrapolation_summary_v8.csv")
    if out_rows:
    ...
    # ------------------------------------------------------------
    txt_path = os.path.join(outdir, "final_theorem_report_v8.txt")
    tex_path = os.path.join(outdir, "final_theorem_report_v8.tex")
    ...
    with open(txt_path, "w", encoding="utf-8") as f:
        f.write("SST ideal-trefoil theorem-style extrapolation report v8\n")
        f.write("=" * 72 + "\n")
        ...
        if A_fit is not None and anc_fit is not None:
        ...
        if lam0_astar_fit is not None and lam0_F_fit is not None:
        ...
        f.write(
            "v8 separates discretization error from contact-model error by fitting the converged tail\n"
            "to y(N)=y_inf + c N^{-alpha}. Exact closure is supported when A_inf -> 1/(4*pi),\n"
            "a_nc/r_c -> 1, and in the shift-free postcheck also a_star/r_c -> 1 and F(1) -> 0.\n"
        )

    with open(tex_path, "w", encoding="utf-8") as f:
        f.write("\\paragraph{v8 asymptotic extrapolation.}\n")
        if A_fit is not None and anc_fit is not None:
            f.write(
                "Using the converged high-resolution tail, we fit "
                "$y(N_{\\mathrm{int}})=y_{\\infty}+cN_{\\mathrm{int}}^{-\\alpha}$. "
            )
            f.write(
                f"For the no-contact preferred branch, "
                f"$A_\\infty={A_fit['y_inf']:.10f}$, "
                f"$A_\\infty-1/(4\\pi)={A_fit['y_inf_minus_target']:.3e}$, "
                f"and $(a_{{\\mathrm{{nc}}}}/r_c)_\\infty={anc_fit['y_inf']:.10f}$ "
                f"with $(a_{{\\mathrm{{nc}}}}/r_c)_\\infty-1={anc_fit['y_inf_minus_target']:.3e}$. "
            )
        if lam0_astar_fit is not None and lam0_F_fit is not None:
            f.write(
                f"For the shift-free postcheck at $\\lambda_K=0$, "
                f"$(a^*/r_c)_\\infty={lam0_astar_fit['y_inf']:.10f}$ "
                f"with $(a^*/r_c)_\\infty-1={lam0_astar_fit['y_inf_minus_target']:.3e}$, "
                f"and $F(1)_\\infty={lam0_F_fit['y_inf']:.3e}$. "
            )
        f.write(
            "These quantities provide the theorem-style numerical closure test in v8."
        )
...
def save_shiftfree_postcheck_plot(shift_rows: List[dict], outdir: str) -> None:
    ...
    plt.legend(fontsize=7, ncol=2)
    plt.tight_layout()
    plt.savefig(os.path.join(outdir, "summary_astar_over_rc_vs_lambda_shiftfree_v8.png"), dpi=170, bbox_inches="tight")
    plt.close(fig)
...
def run_v6_shiftfree_postcheck(rows: List[dict], outdir: str) -> List[dict]:
    ...
    try:
        ...
        csv_path = os.path.join(outdir, "shiftfree_postcheck_v8.csv")
        ...
        txt_path = os.path.join(outdir, "shiftfree_postcheck_v8.txt")
        with open(txt_path, "w", encoding="utf-8") as f:
            f.write("V8 post-sweep shift-free barrier theorem check\n")
            f.write("=" * 64 + "\n")
        ...
        tex_path = os.path.join(outdir, "shiftfree_postcheck_v8.tex")
        with open(tex_path, "w", encoding="utf-8") as f:
            f.write("\\paragraph{V8 post-sweep shift-free barrier check.}\n")
...
def write_run_metadata(outdir: str, mode: str, backend_mode: str, backend_used: str,
    ...
    path = os.path.join(outdir, "run_metadata_v8.json")
...
def main() -> None:
    ...
    global WRITE_V8_EXTRAPOLATION, EXTRAP_MIN_NINT, EXTRA_TARGET_PAIRS

    parser = argparse.ArgumentParser(description="SST Robustness Sweep V8")
    ...
    parser.add_argument("--output-dir", default="", help="Output directory (default: .../sst_ideal_trefoil_robustness_outputs_v8)")
    ...
    parser.add_argument("--extrap-min-nint", type=int, default=8000,
                        help="Minimum N_int_actual included in v8 asymptotic extrapolation")
    parser.add_argument("--no-v8-extrapolation", action="store_true",
                        help="Disable v8 asymptotic extrapolation writer")
    ...
    print("[META] SST ROBUSTNESS SWEEP V8 - mode={} knot_id={}".format(args.mode.upper(), KNOT_ID))
    ...
    for item in geom_configs:
        tmp_gc.append(item)
        existing_gc.add((item[0], item[1], item[2]))
    ...
    csv_path = os.path.join(OUTPUT_DIR, "robustness_summary_v8_{}.csv".format(args.mode))
    ...
    print("[META] Best-estimate v8 CSV/TXT/TeX : {}".format(OUTPUT_DIR))
    if RUN_V6_SHIFT_FREE_POSTCHECK:
        print("[META] Shift-free postcheck v8 CSV/TXT/TeX : {}".format(OUTPUT_DIR))
    print("[META] run_metadata_v8.json : {}".format(os.path.join(OUTPUT_DIR, "run_metadata_v8.json")))

    if WRITE_V8_EXTRAPOLATION:
        try:
            write_v8_extrapolation(rows, postcheck_rows, OUTPUT_DIR)
            print("[META] Extrapolation v8 CSV/TXT/TeX : {}".format(OUTPUT_DIR))
        except Exception as exc:
