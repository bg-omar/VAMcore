...
# 3. Sweep controls
# -----------------------------------------------------------------------------
OUTPUT_DIR = os.path.abspath("./sst_ideal_trefoil_robustness_outputs_v2_renamed")
...
# Optional PyTorch acceleration for the Biot-Savart kernel.
...
# Legacy alias "closest_to_a0" is still accepted for backward compatibility.
...
def _compute_E_BS_norm_torch_impl(
    a_dimless: float,
    pts: np.ndarray,
    tangents: np.ndarray,
    ds_arr: np.ndarray,
...
def choose_root(candidates: List[RootCandidate], a_nc: float, branch_mode: str) -> Tuple[float, bool, float, int, int, str]:
    ...
    if branch_mode == "lowest_energy":
        ...
        # Backward compatibility: any non-"lowest_energy" mode, including the
        # legacy alias "closest_to_a0", is treated as proximity to a_nc.
...
def write_best_estimate(rows: List[dict], outdir: str) -> None:
    ...
    with open(tex_path, "w", encoding="utf-8") as f:
        ...
        f.write("Using the ideal trefoil (Gilbert $3_1$), the preferred current estimate is taken from the selected best no-contact-radius run at the preferred fit window. ")
        ...
        f.write(f"The corresponding no-contact radius estimate is\\n\\[\n\\left(\\frac{{a_0}}{{r_c}}\\right)^{{\\mathrm{{best}}}} = {best['a_nc_over_rc']:.8f} \\pm {a_nc_sys:.8f}_{{\\mathrm{{syst}}}}.\n\\]\n")
        if np.isfinite(a_star_center):
