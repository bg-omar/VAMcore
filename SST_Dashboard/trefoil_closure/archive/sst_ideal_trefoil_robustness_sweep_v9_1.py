#!/usr/bin/env python3
"""
SST ideal-trefoil robustness sweep v9.1
=====================================
...
- make version strings / output filenames consistent with v9.1
...
OUTPUT_DIR = os.path.abspath(os.path.join(SCRIPT_DIR, "sst_ideal_trefoil_robustness_outputs_v9_1"))
KNOT_ID = "3:1:1"
...
def write_best_estimate(rows: List[dict], outdir: str) -> None:
    ...
    csv_path = os.path.join(outdir, "final_best_estimate_v9_1.csv")
    with open(csv_path, "w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(f, fieldnames=list(result.keys()))
        writer.writeheader()
        writer.writerow(result)

    txt_path = os.path.join(outdir, "final_best_estimate_v9_1.txt")
    ...
    tex_path = os.path.join(outdir, "final_best_estimate_v9_1.tex")
    with open(tex_path, "w", encoding="utf-8") as f:
        ...
        f.write("Interpretation: numerical support for percent-level closure; v9.1 inherits v7 dimensionless-x root finding plus v6 theorem/continuation/barrier diagnostics, shift-free postcheck, and optional asymptotic extrapolation reports.\n")
...
def write_v8_extrapolation(rows: List[dict], postcheck_rows: List[dict], outdir: str) -> None:
    ...
    # 3) Write CSV
    # ------------------------------------------------------------
    csv_path = os.path.join(outdir, "extrapolation_summary_v9_1.csv")
    if out_rows:
    ...
    # ------------------------------------------------------------
    txt_path = os.path.join(outdir, "final_theorem_report_v9_1.txt")
    tex_path = os.path.join(outdir, "final_theorem_report_v9_1.tex")
...
def run_v6_shiftfree_postcheck(rows: List[dict], outdir: str) -> List[dict]:
    ...
    try:
        ...
        csv_path = os.path.join(outdir, "shiftfree_postcheck_v9_1.csv")
        ...
        txt_path = os.path.join(outdir, "shiftfree_postcheck_v9_1.txt")
        with open(txt_path, "w", encoding="utf-8") as f:
        ...
        tex_path = os.path.join(outdir, "shiftfree_postcheck_v9_1.tex")
        with open(tex_path, "w", encoding="utf-8") as f:
...
def write_run_metadata(outdir: str, mode: str, backend_mode: str, backend_used: str,
    ...
    path = os.path.join(outdir, "run_metadata_v9_1.json")
...
def main() -> None:
    ...
    parser.add_argument("--output-dir", default="", help="Output directory (default: .../sst_ideal_trefoil_robustness_outputs_v9_1)")
    ...
    for idx, (N_geom, N_int_target, R_ratio) in enumerate(geom_configs):
        ...
        for fr in fit_results:
            if not np.isfinite(fr.A_K):
                ...
                continue
            # v9.1: keep a_nc exactly consistent with the theorem relation
    ...
    print("[META] run_metadata_v9_1.json : {}".format(os.path.join(OUTPUT_DIR, "run_metadata_v9_1.json")))
