...
#!/usr/bin/env python3
"""
Example: build a simple 3D knot curve, extract a PD via vambindings.pd_from_curve,
then compute (or attempt) the hyperbolic volume via vambindings.hyperbolic_volume_from_pd.

Notes
  • If the native C++ hyperbolic solver is not built, the binding delegates to
    `vam_hypvol_no_deps.hyperbolic_volume_from_pd` automatically.
...
try:
    ...
    from vambindings import pd_from_curve, hyperbolic_volume_from_pd
except Exception as e:
    raise SystemExit(
        "ERROR: Could not import 'vambindings'. Build & install the module first.\n"
        f"Details: {e}"
    )
...
def main():
    ap = argparse.ArgumentParser(description="PD + hyperbolic volume demo using vambindings")
