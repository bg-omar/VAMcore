...
n_vectors = 19
xv = np.linspace(-5, 5, n_vectors)
yv = np.linspace(-5, 5, n_vectors)
zv = np.linspace(-5, 5, n_vectors)
Xg, Yg, Zg = np.meshgrid(xv, yv, zv)
points = np.stack((Xg, Yg, Zg), axis=-1).reshape(-1, 3)

# Compute velocity vectors
V = np.array([biot_savart_velocity(p, positions.tolist(), tangents.tolist()) for p in points])
Vmag = np.linalg.norm(V, axis=1)
Vnorm = V / Vmag[:, np.newaxis]

# Compute rotation and pole coloring
