...
void bind_vortex_knot_system(py::module_& m) {
	py::class_<sst::VortexKnotSystem, std::shared_ptr<sst::VortexKnotSystem>>(m, "VortexKnotSystem")
			.def(py::init<>())
			.def("initialize_trefoil_knot", &sst::VortexKnotSystem::initialize_trefoil_knot,
				 py::arg("resolution") = 400,
				 R"pbdoc(
				Initialize a trefoil knot with given resolution (default = 400 points).
			)pbdoc")
			.def("initialize_figure8_knot", &sst::VortexKnotSystem::initialize_figure8_knot,
                 py::arg("resolution") = 400,
                 R"pbdoc(
                    Initialize a figure-eight knot with given resolution (default = 400 points).
                 )pbdoc")

			.def("evolve", &sst::VortexKnotSystem::evolve,
				 py::arg("dt"), py::arg("steps"),
				 R"pbdoc(
				Evolve vortex knot using Biot–Savart dynamics.
			)pbdoc")
			.def("get_positions", &sst::VortexKnotSystem::get_positions,
				 py::return_value_policy::reference,
				 R"pbdoc(
				Get current 3D positions of the knot.
			)pbdoc")
			.def("get_tangents", &sst::VortexKnotSystem::get_tangents,
				 py::return_value_policy::reference,
				 R"pbdoc(
